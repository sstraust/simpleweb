...
(defun simpleweb--display-html-advice-helper (simplify-page-function orig charset url &rest args)
  (let ((target-buffer (current-buffer))
	 ...
	 (with-current-buffer target-buffer
	     ...
	     ;; The buffer eww hands us (from `url-retrieve') is unibyte:
	     ;; inserting a multibyte string there truncates every
	     ;; character to 8 bits, which then shows up as mojibake.
		 ...
		 (insert (encode-coding-string
			  (string-trim (alist-get 'program-directory server-response))
			  coding)))
	       (insert (encode-coding-string
			(alist-get 'modified-page-source server-response)
			coding)))
	     (goto-char start))
